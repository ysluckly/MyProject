...
class HttpResponse
{
  ...
  public:
    ...
    bool ProcessList(RequestInfo& info)
    {
      ...
      //选择文件,name表示属性
      rsp_body += "<input type='text' name='FileUpLoad' value='ABCD' />";
...
>>>>>>> fa09225420aaa12d9497d28b8d0e5408a477b82f
...
=======
      ...
      dup2(in[0],0); //将in[0],重定向0,子进程直接在标准输入读取正文数据
      dup2(out[1], 1); //将out[1]，重定向1，父进程读取正文，直接在标准输出读取就好，即子进程直接打印处理结果传递给父进程
...
>>>>>>> fa09225420aaa12d9497d28b8d0e5408a477b82f
    ...
    bool ProcessCGI(RequestInfo& info)
    {
      ...
      //父进程通过in管道将正文传给子进程，然后通过out管道读取子进程处理结果直到返回0，将处理结果组织http数据，响应客户端 
